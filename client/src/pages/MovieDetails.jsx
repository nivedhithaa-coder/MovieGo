...
useEffect(() => {
    const getShow = async () => {
      ...
      if (!found) {
        // console.log(`Show with id=${id} not found`);
        return;
      }
      setShow({ movie: found, dateTime: dummyDateTimeData });
    };
    getShow();
  }, [id]);
  ...
  return show ?(
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
        <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
            <img src={show?.movie?.poster_path ?? "/fallback.png"} alt="" className='max-ms:mx-auto rounded-xl
             ...
             <div className='relative flex flex-col gap=3'>
                ...
                {/* <p>
                    {show.movie.runtime}.{show.movie.genre.map(genre=> genre.name).join(", ")}.{show.movie.release_Date.split("-")[0]} 
                </p> */}
